...
- Univers de l'expérience : enseobme des résultats possibles (noté $\Omega$)
- Résultat élémentaire : résultat possible de l'expérience (noté $\omega$)
- Ensemble des parties : ensemble de constitué de tous les sous-ensembles possible de $\Omega$ (noté $\mathcal{P}(\Omega)$)
- Evènement (aléatoire) : partie (sous-ensemble) de $\Omega$ (noté $A$)
  - On parle de *réalisation* lorsque l'évènement se produit (*i.e* le résultat $\omega$ appartient au sous-ensembme $A$)
...
- Réunion de $A$ et $B$ : évènement constitué des éléments de $A$ et des éléments de $B$ (ou aux deux donc)
...
- Sous-ensemble de $\Omega$ : **événement**
  - Si ensemble à une seule issue *événement élémentaire*
...
- V.a. *quantitative* si les valeurs de $X$ sont quantitatives
...
La *fonction de masse* de la v.a. associe une probabilité $P(X \le x_i)$ à chaque résultat élémentaire $x_i$
...
```{r ex1-bis}
tab = as.data.frame(table(gain), responseName = "p")
kable(t(tab)) %>%
  kable_styling(position = "center")
```
...
```{r ex1-ter, out.width='90%', fig.align='center'}
...
$\mathbf{X}$ suit une **loi discrète** si pour chaque $i$, $P(X = k) = \frac{1}{n}$.
...
$\mathbf{X}$ suit une **loi de Poisson** $Pois(\lambda)$ si pour chaque $i$, $P(X = k) = \frac{\lambda^k}{k!}e^{-k}$ où
